...
>
> **0005는 몇 번 실행해도 안전합니다.** 실패하면 어디까지 적용됐는지 추적하지
> 말고 원인만 고쳐 파일 전체를 그대로 다시 붙여넣으세요. 모든 문장이
> `if exists` / `if not exists` 이거나 이미 적용된 상태에서 0행을 건드립니다.
> (실제로 필요했습니다 — SQL 에디터에서 같은 파일이 한 번은 롤백되고 한 번은
> 일부만 남았습니다. 에디터가 문장을 어떻게 쪼개는지에 기대지 않는 편이 낫습니다.)
>
> 같은 날 같은 반에 설계가 두 개 있으면 정리한 뒤 고유 인덱스를 겁니다. 남기는
