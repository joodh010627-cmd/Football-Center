...
> **2026-09-26 변경:** 문의(leads)·공개 폼 링크는 실제 테이블로 옮겼고(0006), 학부모가
> 로그인 없이 실제로 접수할 수 있다. 알림톡은 "발송 직전"까지 — 대기열·심사용 템플릿·
> 대행사 어댑터·드라이런 발송 서버 — 지었다. 실발송(카카오 채널·대행사 계약)은 여전히 남아 있다.
> 절차는 [`ALIMTALK.md`](ALIMTALK.md).
